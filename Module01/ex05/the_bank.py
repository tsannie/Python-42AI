    ...
    def fix_account(self, name):
        ...
        for attribute in account_to_fix.__dict__:
            ...
            if attribute.startswith('zip'):
                zip_present = True
            if attribute.startswith('addr'):
                addr_present = True

        if not zip_present:
            account_to_fix.zip = '000-000'
        if not addr_present:
            account_to_fix.addr = 'Unknown'

        if len(account.__dict__) % 2:
            for attribute in account_to_fix.__dict__:
                if attribute != 'name' and attribute != 'id' and attribute != 'value' \
                  or not attribute.startswith('zip') or not attribute.startswith('addr'):
                    delattr(account_to_fix, attribute)
                    break
        return True
